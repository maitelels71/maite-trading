...
class MagnetMa20GapStrategy(BaseStrategy):
    ...
    def evaluate(
        ...
        return evaluate_each_session_day(
            context, tz=tz, candles_for_days=m15, score_day=score
        )
    ...
    *,
    ...
    for i in reversed(prior_idxs):
        ...
        if side is None:
            ...
            break
        days.add(local_ts(h1[i].timestamp, tz).date())
...
def _last_ma_before_session(
    ...
    *,
    ...
    for i in range(len(h1) - 1, -1, -1):
        if local_ts(h1[i].timestamp, tz).date() < session_day and series[i] is not None:
            return series[i]
    return None
